...
?>
<div class="wrap flavor-like-pulse-upgrade">
	...
	<p class="submit" style="margin-top:1.5em;">
		<button type="button" class="button button-primary" id="flavor-like-pulse-drop-legacy">
			<?php echo esc_html( 'Remove old tables' ); ?>
		</button>
		<button type="button" class="button" id="flavor-like-pulse-dismiss">
	...
	<p class="description" style="max-width:560px;margin-top:0.5em;font-size:12px;">
		<?php echo esc_html( 'For details, an administrator can run: wp flavor_like pulse verify --deep' ); ?>
	</p>
	<p class="submit" style="margin-top:1.5em;">
		<button type="button" class="button button-primary" id="flavor-like-pulse-dismiss">
	...
	<p class="description" style="max-width:560px;margin-top:0.5em;">
		<a href="<?php echo esc_url( Flavor_Like_Pulse_Admin::get_help_url() ); ?>"><?php echo esc_html( '← Back to Overview' ); ?></a>
	</p>
	...
	<div class="notice notice-success inline" id="flavor-like-pulse-next-step" style="max-width:560px;margin-top:1.5em;padding:12px;">
		<p style="margin:0;">
	...
	<table class="widefat striped" style="max-width:560px;margin-top:1.5em;">
		<tbody>
			<tr>
				...
				<td><span id="flavor-like-pulse-sync-status"><?php echo esc_html( $status_label ); ?></span></td>
			</tr>
			<tr>
				<th scope="row"><?php echo esc_html( 'Progress' ); ?></th>
				<td id="flavor-like-pulse-progress-text"><?php echo esc_html( $progress_label ); ?></td>
			</tr>
		</tbody>
	</table>

	<div style="max-width:560px;margin-top:0.75em;background:#f0f0f1;border-radius:4px;overflow:hidden;height:8px;">
		<div id="flavor-like-pulse-progress-bar" style="width:<?php echo esc_attr( (string) $percent ); ?>%;height:100%;background:#2271b1;transition:width 0.3s;"></div>
	</div>

	<p class="submit" style="margin-top:1.5em;">
		<?php if ( $show_start ) : ?>
		<button type="button" class="button button-primary" id="flavor-like-pulse-start" <?php disabled( $is_running ); ?>>
			<?php echo esc_html( 'Start upgrade' ); ?>
		</button>
		<?php endif; ?>
		<button type="button" class="button" id="flavor-like-pulse-pause" <?php disabled( ! $is_running ); ?><?php echo $show_start ? '' : ' style="display:none;"'; ?>>
			<?php echo esc_html( 'Pause' ); ?>
		</button>
		<?php if ( $can_enable ) : ?>
		<button type="button" class="button<?php echo $sync_complete ? ' button-primary' : ''; ?>" id="flavor-like-pulse-enable" <?php disabled( ! $sync_complete ); ?>>
	...
	<div id="flavor-like-pulse-log" style="max-width:560px;margin-top:1em;font-size:13px;color:#646970;"></div>

	<?php if ( $show_migrate ) : ?>
	<details class="flavor-like-pulse-cli" style="max-width:560px;margin-top:2em;">
