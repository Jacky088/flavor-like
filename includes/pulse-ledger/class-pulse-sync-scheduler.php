 ...
 * Action Scheduler (WooCommerce et al.) — Flavor Like is not an e-commerce
...
if ( ! class_exists( 'Flavor_Like_Pulse_Sync_Scheduler' ) ) {

	final class Flavor_Like_Pulse_Sync_Scheduler {

		const HOOK = 'flavor_like_pulse_sync_batch';
		...
		public static function run_scheduled_batch() {
			$result = Flavor_Like_Pulse_Sync::run_batch();

			if ( empty( $result['done'] ) && Flavor_Like_Pulse_Config::migration_running() ) {
				self::schedule();
			}
		}
	}
