 ...
 *
 * Not for item meta rows (flavor_like_{type}_meta) or third-party page-cache purge.
 *
 * @package Flavor_Like
 */
...
if ( ! class_exists( 'Flavor_Like_Query_Cache' ) ) {

	final class Flavor_Like_Query_Cache {

		const VERSION_OPTION            = 'flavor_like_pulse_cache_ver';
		const STATS_ITEM_ID             = 1;
		const STATS_META_GROUP          = 'statistics';
		const ADMIN_NEW_VOTES_KEY       = 'calculate_new_votes';
		const STATS_GROUP               = 'flavor-like-stats';
		...
		public static function key( $logical_key ) {
			...
			$scope       = flavor_like_pulse_mode() . '_' . flavor_like_pulse_read_mode();

			if ( Flavor_Like_Pulse_Config::MODE_DUAL === flavor_like_pulse_mode() ) {
		...
		public static function should_defer_bump() {
			if ( class_exists( 'Flavor_Like_Pulse_Writer' ) && Flavor_Like_Pulse_Writer::is_migrating() ) {
				return true;
			}

			return Flavor_Like_Pulse_Config::migration_running();
		}
		...
		public static function rebuild_statistics_meta() {
			if ( ! class_exists( 'Flavor_Like_Pulse_Query' ) || ! Flavor_Like_Pulse_Query::available() ) {
				return;
			}

			self::set_statistics_meta(
				'count_logs_period_all',
				Flavor_Like_Pulse_Query::count_logs_for_mode( 'all' )
			);

			foreach ( Flavor_Like_Pulse_Registry::stats_table_map() as $item_type ) {
				self::set_statistics_meta(
					sprintf( 'count_logs_for_%s_in_all_daterange', $item_type ),
					Flavor_Like_Pulse_Query::count_logs_for_type( $item_type, 'all' )
				);
			}
		}
		...
		public static function purge_stale_statistics_meta() {
			if ( ! class_exists( 'Flavor_Like_Meta_Schema' ) || ! Flavor_Like_Meta_Schema::table_exists() ) {
				return;
			}

			global $wpdb;

			$table = esc_sql( Flavor_Like_Meta_Schema::table() );
		...
		public static function get_statistics_meta( $logical_key ) {
			return flavor_like_get_meta_data( self::STATS_ITEM_ID, self::STATS_META_GROUP, sanitize_key( (string) $logical_key ), true );
		}
		...
		public static function set_statistics_meta( $logical_key, $value ) {
			return flavor_like_update_meta_data( self::STATS_ITEM_ID, self::STATS_META_GROUP, sanitize_key( (string) $logical_key ), $value );
		}
		...
		public static function increment_admin_new_votes() {
			...
			$current = flavor_like_get_meta_data( self::STATS_ITEM_ID, self::STATS_META_GROUP, self::ADMIN_NEW_VOTES_KEY, true );
			if ( ! is_numeric( $current ) ) {
				$current = 0;
			}

			flavor_like_update_meta_data( self::STATS_ITEM_ID, self::STATS_META_GROUP, self::ADMIN_NEW_VOTES_KEY, (int) $current + 1 );
		}
		...
		public static function reset_admin_new_votes() {
			flavor_like_update_meta_data( self::STATS_ITEM_ID, self::STATS_META_GROUP, self::ADMIN_NEW_VOTES_KEY, 0 );
		}
	}
}
