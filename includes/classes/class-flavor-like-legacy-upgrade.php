...
if ( ! class_exists( 'Flavor_Like_Legacy_Upgrade' ) ) {

	final class Flavor_Like_Legacy_Upgrade {
		...
		private static function ensure_meta_item_id_column() {
			if ( ! Flavor_Like_Meta_Schema::table_exists() || ! self::column_exists( Flavor_Like_Meta_Schema::table(), 'item_id' ) ) {
				return true;
			}

			$meta = esc_sql( Flavor_Like_Meta_Schema::table() );
		...
		private static function run_query( $query ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'Flavor Like: Legacy upgrade query failed - ' . $wpdb->last_error . ' | Query: ' . $query );
			}
