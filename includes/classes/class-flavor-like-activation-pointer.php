<?php
/**
 * Welcome pointer on the Flavor Like admin menu after activation.
 *
...
if ( ! class_exists( 'Flavor_Like_Activation_Pointer' ) ) {

	/**
	 * Shows a one-time getting-started pointer on the site admin menu.
	 */
	class Flavor_Like_Activation_Pointer {

		const USER_META_KEY   = 'flavor_like_show_activation_pointer';
		const AJAX_ACTION     = 'flavor_like_dismiss_activation_pointer';
		const SCRIPT_HANDLE   = 'flavor-like-activation-pointer';
		const STYLE_HANDLE    = 'flavor-like-activation-pointer';
		const MENU_SELECTOR   = '#toplevel_page_flavor-like-settings';
		...
		protected static function user_has_menu_capability( $user_id = 0 ) {
			...
			return user_can( $user_id, 'manage_options' ) || user_can( $user_id, flavor_like_get_user_access_capability( 'stats' ) );
		}
		...
		public static function enqueue_assets( $hook ) {
			...
			$css_path = FLAVOR_LIKE_ADMIN_DIR . '/assets/css/activation-pointer.css';
			$js_path  = FLAVOR_LIKE_ADMIN_DIR . '/assets/js/activation-pointer.js';
			$css_ver  = file_exists( $css_path ) ? (string) filemtime( $css_path ) : FLAVOR_LIKE_VERSION;
			$js_ver   = file_exists( $js_path ) ? (string) filemtime( $js_path ) : FLAVOR_LIKE_VERSION;

			wp_enqueue_style(
				self::STYLE_HANDLE,
				FLAVOR_LIKE_ADMIN_URL . '/assets/css/activation-pointer.css',
				array(),
				$css_ver
			);
			...
			wp_enqueue_script(
				self::SCRIPT_HANDLE,
				FLAVOR_LIKE_ADMIN_URL . '/assets/js/activation-pointer.js',
				array(),
				$js_ver,
				true
			);
		...
		public static function render_template() {
			...
			$template = FLAVOR_LIKE_ADMIN_DIR . '/includes/templates/activation-pointer-dialog.php';
		...
		/**
		 * Network admin has no Flavor Like menu — guide users to a site dashboard.
		 *
		 * @return void
		 */
		public static function maybe_network_notice() {
			...
			printf(
				...
				esc_html__( 'Flavor Like is active on your network.', 'flavor-like' ),
				esc_html__( 'Open a site dashboard to see the getting-started guide next to the Flavor Like menu.', 'flavor-like' ),
				esc_url( $dashboard_url ),
				esc_html__( 'Go to site dashboard', 'flavor-like' )
			);
		}
