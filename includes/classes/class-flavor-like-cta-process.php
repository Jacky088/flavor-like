...
if ( ! class_exists( 'flavor_like_cta_process' ) ) {

	class flavor_like_cta_process extends flavor_like_entities_process {
		...
		function __construct( $atts ){
			...
			// Get settings type
			$this->settings   = flavor_like_setting_type::get_instance( $this->parsedArgs['item_type'] );

			parent::__construct( array(
				'user_id'     => $this->parsedArgs['user_id'],
				'user_ip'     => $this->parsedArgs['user_ip'],
				'item_type'   => $this->parsedArgs['item_type'],
				'item_method' => flavor_like_setting_repo::getMethod( $this->parsedArgs['item_type'] )
			) );
		}
		...
		public function update(){
			...
			// Get logging method
			$logging_method = flavor_like_setting_repo::getMethod( $this->parsedArgs['item_type'] );
			// Set current status
		...
		public function getAjaxProcessAtts(){
			return apply_filters( 'flavor_like_ajax_process_atts', array(
					...
					"logging_method"       => flavor_like_setting_repo::getMethod( $this->parsedArgs['item_type'] ),
					"only_logged_in_users" => flavor_like_setting_repo::requireLogin( $this->parsedArgs['item_type'] ),
					"logged_out_action"    => flavor_like_setting_repo::anonymousDisplay( $this->parsedArgs['item_type'] ),
		...
		public function getCounterValue(){
			$counter_val = flavor_like_get_counter_value( $this->parsedArgs['item_id'], $this->parsedArgs['item_type'], $this->getCurrentStatus(), $this->isDistinct() );

			// Hide if zero
			if( flavor_like_setting_repo::isCounterZeroHidden( $this->parsedArgs['item_type'] ) && $counter_val == 0 ){
				$counter_val = '';
			} else {
				$counter_val = flavor_like_format_number( $counter_val, $this->getCurrentStatus() );
			}

			return apply_filters( 'flavor_like_ajax_counter_value', $counter_val, $this->parsedArgs['item_id'], $this->parsedArgs['item_type'], $this->getCurrentStatus(), $this->isDistinct(), $this->parsedArgs['item_template'] );
		}
