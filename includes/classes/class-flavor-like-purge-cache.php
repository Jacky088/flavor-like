<?php
/**
 * Flavor Like purge plugin cache
 * // @echo HEADER
*/
...
if ( ! class_exists( 'flavor_like_purge_cache' ) ) {
	/**
	 *  Class to purge third-party plugins cache
	 */
	class flavor_like_purge_cache {
		...
		protected function purgeNitropackCache($post_ids = [], $reffer_url = NULL)
		{
			...
			if (empty($post_ids)) {
				nitropack_invalidate(null, null, 'Invalidating all pages after user engagement (likes/dislikes) via Flavor Like.');
				return;
			}
			foreach ($post_ids as $postId) {
				...
				if (in_array($postType, $cacheableTypes)) {
					nitropack_invalidate(null, "single:{$postId}", sprintf('Invalidating "%s" after user engagement (likes/dislikes) via Flavor Like.', $postTitle));
				}
			}
		}
