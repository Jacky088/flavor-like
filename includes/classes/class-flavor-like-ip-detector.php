...
if ( ! class_exists( 'Flavor_Like_Ip_Detector' ) ) {
	...
	class Flavor_Like_Ip_Detector {
		 ...
		 * "Trusted Proxy IPs" option (or the flavor_like_trusted_proxies filter)
			...
			if ( ! empty( $cf_ip ) ) {
				...
				if ( ! empty( $remote_addr ) && self::is_cloudflare_ip( $remote_addr ) ) {
					if ( self::validate_ip( $cf_ip ) ) {
						$ip = $cf_ip;
						self::$cached_ip = apply_filters( 'flavor_like_get_user_ip', $ip );
						return self::$cached_ip;
					}
				}
			}
		...
		private static function get_trusted_proxies() {
			...
			$raw = flavor_like_get_option( 'trusted_proxy_ips', '' );
			$list = array();
			 ...
			 */
			$list = apply_filters( 'flavor_like_trusted_proxies', $list );
		...
		public static function get_cloudflare_ips() {
			...
			$transient_key = 'flavor_like_cloudflare_ips';
