...
if ( ! class_exists( 'flavor_like_entities_process' ) ) {

	class flavor_like_entities_process {
		...
		protected function setCurrentIP( $user_ip ){
			if ( $user_ip === NULL ) {
				...
				if ( $cached_ip === null ) {
					$cached_ip = flavor_like_get_user_ip();
				}
		...
		protected function setTypeSettings(){
			...
			$this->typeSettings = flavor_like_setting_type::get_instance( $this->itemType );
		}
		...
		protected function setCurrentUser( $user_id ){
			if( $user_id === NULL ){
				$this->currentUser = $this->isUserLoggedIn ? get_current_user_id() : flavor_like_generate_user_id( $this->currentIP );
			} else {
		...
		public function getCurrentFingerPrint(){
			if ( $this->currentFingerPrint === null ) {
				$this->currentFingerPrint = flavor_like_generate_fingerprint();
			}
		...
		public function setCurrentStatus( $args, $keep_status = false, $force_status = false ){
			...
			$this->currentStatus = apply_filters( 'flavor_like_user_current_status', $this->currentStatus, $this->prevStatus, $args );
		}

		/**
		 * Set user previous status
		 * Note: flavor_like_get_user_item_history already uses WordPress meta cache internally
		 *
		...
		public function setPrevStatus( $item_id ){
			...
			// get user log array
			$get_user_history = flavor_like_get_user_item_history( $args );
			// set prev status
			$this->prevStatus = apply_filters( 'flavor_like_user_prev_status', ! empty( $get_user_history[$item_id] ) ? $get_user_history[$item_id] : false, $args );
		}
		...
		public static function hasPermission( $args, $settings ){
			...
			if ( $cached_is_bot === null ) {
				$cached_is_bot = flavor_like_is_bot_request();
			}
			if ( $cached_is_bot ) {
				return false;
			}
			...
			if ( ! isset( $cached_methods[ $args['type'] ] ) ) {
				$cached_methods[ $args['type'] ] = flavor_like_setting_repo::getMethod( $args['type'] );
			}
			...
			if ( in_array( $method, array( 'by_cookie', 'by_user_ip_cookie' ) ) ) {
				$cookie_key = sanitize_key( 'flavor_like_' . md5( $args['type'] . '_logs' ) );
				...
				|| ( '' !== ( $legacy_cookie_base = self::getLegacyCookieBase( $settings->getCookieName() ) ) && isset( $_COOKIE[ $legacy_cookie_base . $args['item_id'] ] ) ) ) {
					$status = false;
					$has_cookie = true;
				}
				...
				if ( ! $has_cookie && $args['method'] === 'process' ) {
					$cookie_expire = flavor_like_setting_repo::getCookieExpiration( $args['type'] );
					...
					flavor_like_setcookie( $cookie_key, wp_json_encode( $cookie_data ), time() + 2147483647 );
				}
			}

			// Fingerprint check for guests or requests without cookies
			if ( $args['method'] === 'process' && in_array( $method, ['do_not_log', 'by_cookie'] ) ) {
				$current_finger_print = empty( $args['current_finger_print'] ) ? flavor_like_generate_fingerprint() : $args['current_finger_print'];

				$fingerprint_count = flavor_like_count_current_fingerprint(
				...
				if ( ! empty( $fingerprint_count ) ) {
					if ( $method === 'do_not_log' ) {
						if ( $fingerprint_count >= flavor_like_setting_repo::getVoteLimitNumber( $args['type'] ) ) {
							$status = false;
						}
			...
			return apply_filters( 'flavor_like_permission_status', $status, $args, $settings );
		}
		...
		private static function getDecodedCookieData( $cookie_key ) {
			...
			if ( ! isset( $cached_decoded_cookies[ $cookie_key ] ) ) {
				...
				$cookie_data = is_array( $cookie_data ) ? $cookie_data : array();

				// Fall back to the pre-rename cookie (wp_ulike_*) so guests who
				// voted before the 1.0.6 rebrand are still recognised.
				if ( empty( $cookie_data ) ) {
					$legacy_key = self::getLegacyCookieBase( $cookie_key );
					if ( '' !== $legacy_key && isset( $_COOKIE[ $legacy_key ] ) ) {
						$legacy_data = json_decode( wp_unslash( $_COOKIE[ $legacy_key ] ), true );
						if ( is_array( $legacy_data ) ) {
							$cookie_data = $legacy_data;
						}
					}
				}

				$cached_decoded_cookies[ $cookie_key ] = $cookie_data;
			}
			return $cached_decoded_cookies[ $cookie_key ];
		}

		/**
		 * Map a flavor_like_* cookie name to its pre-1.0.6 wp_ulike_* name.
		 *
		 * @param string $cookie_name Current cookie base name.
		 * @return string Legacy name, or '' when not applicable.
		 */
		private static function getLegacyCookieBase( $cookie_name ) {
			if ( is_string( $cookie_name ) && 0 === strpos( $cookie_name, 'flavor_like_' ) ) {
				return 'wp_ulike_' . substr( $cookie_name, strlen( 'flavor_like_' ) );
			}
			return '';
		}
		...
		public function isDistinct(){
			return flavor_like_setting_repo::isDistinct( $this->itemType );
		}
		...
		public function insertData( $item_id ){
			if ( flavor_like_writes_pulse() ) {
				$item_type = Flavor_Like_Pulse_Registry::from_setting_type( $this->typeSettings->getType() );
				$payload   = array(
					'item_id'       => $item_id,
					'item_type'     => $item_type,
				...
				return $this->isDistinct()
					? Flavor_Like_Pulse_Writer::upsert( $payload )
					: Flavor_Like_Pulse_Writer::insert( $payload );
			}
	...
	if ( false !== $row ) {
		do_action( 'flavor_like_data_inserted', [
		...
		protected function maybeAnonymiseIp( $ip ){
			...
			if( flavor_like_setting_repo::isAnonymiseIpOn() ){
				if( flavor_like_setting_repo::isIpLoggingOff() ){
					$ip = '0.0.0.0';
				} else {
					if ( strpos( $ip, "." ) !== false ) {
			...
			return apply_filters( 'flavor_like_database_user_ip', esc_sql( $ip ) );
		}
		...
		public function updateData( $item_id ){
			if ( flavor_like_writes_pulse() ) {
				$item_type = Flavor_Like_Pulse_Registry::from_setting_type( $this->typeSettings->getType() );
				return Flavor_Like_Pulse_Writer::upsert(
					array(
						'item_id'       => $item_id,
						'item_type'     => $item_type,
						'setting_type'  => $this->typeSettings->getType(),
		...
		do_action( 'flavor_like_data_updated', [
	...
	public function deleteData( $item_id ){
		if ( flavor_like_writes_pulse() ) {
			$item_type = Flavor_Like_Pulse_Registry::from_setting_type( $this->typeSettings->getType() );
			return Flavor_Like_Pulse_Writer::delete( $item_id, $item_type, $this->currentUser );
		}
	...
	if ( $deleted ) {
		do_action(
			...
			absint( $item_id ),
			Flavor_Like_Pulse_Registry::from_setting_type( $this->typeSettings->getType() ),
			$this->typeSettings,
			(int) $deleted
		);
	}
	...
	public function updateCounterMeta( $item_id ){
			...
			if( flavor_like_is_cache_exist() && $item_id ){
				wp_cache_delete( $item_id, sprintf( 'flavor_like_%s_meta', $this->itemType ) );
			}

			// Remove 'un' prefix from status.
			$status  = ltrim( $this->currentStatus, 'un');
			// Get current value
			$pre_val = flavor_like_meta_counter_value( $item_id, $this->itemType, $status, $this->isDistinct() );
			...
			if( ! is_null( $pre_val ) ){
				...
				// Update meta
				flavor_like_update_meta_counter_value( $item_id, $pre_val, $this->itemType, $status, $this->isDistinct() );
			}
			...
			if( $this->isDistinct() && $this->prevStatus ){
				...
				if( ltrim( $this->prevStatus, 'un') !== $status &&
					...
					$reverse_val = flavor_like_meta_counter_value( $item_id, $this->itemType, $reverse_key, $this->isDistinct() );
					// Update meta if exist
					if( ! is_null( $reverse_val ) ){
						flavor_like_update_meta_counter_value( $item_id, max( $reverse_val - 1, 0 ), $this->itemType, $reverse_key, $this->isDistinct() );
					}
				}
			}
		}
		...
		public function updateUserMetaStatus( $item_id ){
			...
			// delete cache to get fresh data
			if( flavor_like_is_cache_exist() && $this->currentUser ){
				wp_cache_delete( $this->currentUser, 'flavor_like_user_meta' );
			}
			// Get meta data
			$user_info = flavor_like_get_meta_data( $this->currentUser, 'user', $meta_key, true );
			...
			// Update meta value
			flavor_like_update_meta_data( $this->currentUser, 'user', $meta_key, $user_info );
		}
		...
		public function updateLikerMetaList( $item_id ){
			...
			// Get meta data
			$get_likers = flavor_like_get_meta_data( $item_id, $this->itemType, 'likers_list', true );
			...
			if( $get_user ){
				...
				if( $is_updated ){
					flavor_like_update_meta_data( $item_id, $this->itemType, 'likers_list', $get_likers );
				}
			}
		}
