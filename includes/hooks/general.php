...
if( ! function_exists( 'wp_ulike_put_posts' ) ){
	...
	function wp_ulike_put_posts( $content ) {
		...
		if ( WpUlikeInit::is_admin_backend() || WpUlikeInit::is_cron() || WpUlikeInit::is_rest() ) {
			return apply_filters( 'wp_ulike_the_content', $content, $content );
		}
		...
		$hide_list = wp_ulike_get_option( 'posts_group|auto_display_filter', null );
		if ( null === $hide_list ) {
			// 未保存过设置，使用默认值：除了 single 以外都隐藏
			$hide_list = wp_ulike_setting_repo::getAutoDisplayFilterDefault();
		}
		...
		if ( ! $should_display && is_singular() ) {
			$allowed_post_types = wp_ulike_setting_repo::getPostTypesFilterList();
