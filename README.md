...
- 投票互斥锁由文件锁（flock + unlink）改为 MySQL `GET_LOCK`，消除并发场景下的重复计数风险
- 锁释放改为 `try/finally` 结构，异常路径下也能保证释放
- `flavor_like_get_likers` AJAX 端点补上 nonce 校验
- 新增受信代理 IP 白名单，防止伪造 `X-Forwarded-For` 绕过 IP 去重与黑名单
- Cloudflare IP 列表拉取失败时仅缓存 5 分钟

**性能优化**

- 新增资源加载策略：按需模式下仅含点赞按钮的页面输出 CSS/JS
