...
if ( ! class_exists( 'flavor_like_notices' ) ) {
    class flavor_like_notices{
        ...
        function __construct( $args = array() ){
            $defaults   = array(
                ...
                'buttons'        => array(),
                'dismissible'    => array(
                    'url_key'    => 'flavor-like-hide-core-plugin-notice',
                    'action'     => 'flavor_like_hide_notices_nonce',
            ...
            if( empty( $this->args['id'] ) ){
                return new WP_Error( 'missing_id', esc_html__( "You need to enter a unique id for notice.", 'flavor-like' ) );
            }
         ...
         * @return void | string
         */
        private function get_image( $before = '<div class="flavor-like-notice-image">', $after = '</div>' ){
         ...
         * @return void | string
         */
        private function get_title( $before = '<h3 class="flavor-like-notice-title">', $after = '</h3>' ){
         ...
         * @return void | string
         */
        private function get_skin( $prefix = 'flavor-like-notice-skin-' ){
         ...
         * @return void | string
         */
        private function get_description( $before = '<p class="flavor-like-notice-description">', $after = '</p>' ){
        ...
        private function get_buttons(){
            ...
            $default_args = [
                ...
                'extra_classes' => 'flavor-like-notice-btn'
            ];
            ...
            foreach ( $this->args['buttons'] as $btn_args ) {
                ...
                if( !empty( $btn_args['type']  ) && 'skip' === $btn_args['type'] ){
                    $current_default_args['extra_classes'] .= ' flavor-like-skip-notice flavor-like-btn-ghost';
                    $current_default_args['link']           = '';
                } else {
                    $current_default_args['extra_classes'] = 'flavor-like-notice-cta-btn';
                }
                ...
                $this->buttons .= flavor_like_widget_button_callback( $btn_args );
            }
    ...
    ?>
            <a href="<?php echo esc_url( $this->get_nonce_url() ); ?>" class="notice-dismiss flavor-like-skip-notice flavor-like-close-notice">
                <span class="screen-reader-text"><?php esc_html_e( 'Skip', 'flavor-like' ); ?></span>
            </a>
    <?php }
        ...
        private function flush_dismissible(){
            if ( isset( $_GET[ $this->args['dismissible']['url_key'] ] ) && isset( $_GET[ '_notice_nonce' ] ) && $_GET[ $this->args['dismissible']['url_key'] ] === $this->args['id'] ) {
                if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET[ '_notice_nonce' ] ) ),  $this->args['dismissible']['action'] ) ) {
                    wp_die( esc_html__( 'Authorization failed. Please refresh the page and try again.', 'flavor-like' ) );
                }
                flavor_like_set_transient( $this->get_transient_key(), 1, $this->args['dismissible']['expiration'] );
        ...
        private function is_dismissible(){
            if( ! is_array( $this->args['dismissible'] ) || flavor_like_get_transient( $this->get_transient_key() ) ){
                return true;
            }
            return false;
        }
        ...
        private function is_snoozed(){
            if( ! empty( $this->args['initial_snooze'] ) ){
                ...
                $snooze_time   = flavor_like_get_transient( $transient_key );
                if( $snooze_time && $snooze_time > strtotime( "now" ) ){
                    return true;
                } elseif( $snooze_time === false ) {
                    flavor_like_set_transient( $transient_key, strtotime( $this->args['initial_snooze'] . " seconds" ) );
                    return true;
                }
            }
            return false;
        ...
        private function get_transient_key(){
            return 'flavor-like-notice-' . esc_attr($this->args['id']);
        }
        ...
        private function get_unique_class(){
            return 'flavor-like-notice-id-' . esc_attr( $this->args['id'] );
        }
        ...
        public function render(){
            ...
            echo sprintf(
                '<div class="notice flavor-like-notice flavor-like-notice-control flavor-like-notice-wrapper %1$s %2$s" %3$s data-notice-id="%4$s" data-dismiss-nonce="%5$s" data-dismiss-expiration="%6$s">%7$s<div class="flavor-like-notice-info">%8$s%9$s<div class="flavor-like-notice-submit">%10$s</div></div>%11$s</div>',
