...
if ( ! class_exists( 'flavor_like_stats' ) ) {

	class flavor_like_stats extends flavor_like_widget{
		...
		function __construct(){
			global $wpdb;
			$this->wpdb   = $wpdb;
			$this->tables = Flavor_Like_Pulse_Registry::stats_table_map();
		}
		...
		public function get_all_data() {
			...
			$meta   = flavor_like_get_site_stats_meta( array_keys( $tables ) );
	...
	public function select_data( $item_type ) {
		$data_limit  = max( 1, absint( apply_filters( 'flavor_like_stats_data_limit', 30 ) ) );
		$logical_key = sprintf( 'stats_chart_%s_%d', sanitize_key( $item_type ), $data_limit );

		return Flavor_Like_Query_Cache::remember_stats(
			$logical_key,
			static function () use ( $item_type, $data_limit ) {
				return Flavor_Like_Pulse_Log_Bridge::get_chart_dataset( $item_type, $data_limit );
			}
		);
	}
		...
		public function count_all_logs( $date = 'all' ){
			return flavor_like_count_all_logs( $date );
		}
		...
		public function count_logs( $args = array() ){
			...
			$resolved_type = Flavor_Like_Pulse_Registry::type_by_table_suffix( $item_type );
			$item_type     = $resolved_type ? $resolved_type : Flavor_Like_Pulse_Registry::normalize_item_type( $item_type );
			...
			$counter_value = Flavor_Like_Query_Cache::remember_stats(
				...
				static function () use ( $item_type, $date ) {
					return Flavor_Like_Pulse_Query::count_logs_for_type( $item_type, $date );
				}
			);
		...
		public function top_posts() {
			...
			$item_info = flavor_like_get_popular_items_info( array(
				'type'     => 'post',
				'rel_type' => $post_type,
				'status'   => 'like',
				'period'   => 'all',
			...
			$posts = get_posts( apply_filters( 'flavor_like_get_top_posts_query', array(
				'post_type'      => $post_type,
		...
		public function top_comments() {
			...
			$comments = get_comments( apply_filters( 'flavor_like_get_top_comments_query', array(
				'comment__in' => $ids,
				'orderby'     => 'comment__in',
				'post_type'   => $post_type,
			) ) );
			...
			foreach ( $comments as $comment ) {
				...
				$context      = sprintf(
					...
					__( '%1$s on %2$s', 'flavor-like' ),
					$author,
					$post_title ? $post_title : __( '(Untitled post)', 'flavor-like' )
				);

				if ( empty( $excerpt ) ) {
					$title = sprintf(
						/* translators: %s: comment author and post context, e.g. "John on Hello World" */
						__( 'Comment by %s', 'flavor-like' ),
						$context
					);
				} else {
		...
		public function get_peak_hours() {
			...
			return Flavor_Like_Query_Cache::remember_stats(
				...
				static function () use ( $tables ) {
					$results = Flavor_Like_Pulse_Log_Bridge::get_peak_hours_rows( array_values( $tables ) );
				...
				Flavor_Like_Query_Cache::TTL_PEAK_HOURS
			);
		}
