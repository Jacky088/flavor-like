<?php
/**
 * Flavor Like Customizer REST API
 *
...
if ( ! class_exists( 'flavor_like_customizer_api' ) ) {
    class flavor_like_customizer_api {
        ...
        public function get_schema( $request = null ) {
            ...
            return apply_filters( 'flavor_like_optiwich_customizer_schema', $schema );
        }
        ...
        protected function get_plugin_assets() {
            ...
            $ver = defined( 'FLAVOR_LIKE_VERSION' ) ? FLAVOR_LIKE_VERSION : '1.0.0';
            $assets['css'][] = array(
                'url'    => add_query_arg( 'ver', $ver, FLAVOR_LIKE_ASSETS_URL . '/css/flavor-like.min.css' ),
                'source' => 'free',
                'ver'    => $ver,
            );
            // Preview-only layout helpers (gallery labels, scroll, non-interactive buttons).
            $assets['css'][] = array(
                'url'    => add_query_arg( 'ver', $ver, FLAVOR_LIKE_ASSETS_URL . '/css/customizer-preview.css' ),
                'source' => 'free',
                'ver'    => $ver,
            );
            $assets['js'][] = array(
                'url'    => add_query_arg( 'ver', $ver, FLAVOR_LIKE_ASSETS_URL . '/js/flavor-like.min.js' ),
                'source' => 'free',
                'ver'    => $ver,
            );

            // Add free plugin localized script
            $assets['localized_scripts']['flavor_like_params'] = array(
                'data' => array(
            ...
            $assets = apply_filters( 'flavor_like_customizer_assets', $assets );
        ...
        protected function build_schema_from_customizer() {
            ...
            return array( 'pages' => apply_filters( 'flavor_like_optiwich_customizer_pages', $pages ) );
        }
        ...
        protected function get_customizer_sections() {
            ...
            $customizer = new flavor_like_customizer();
        ...
        public function build_pages_structure( $sections ) {
            ...
            if ( ! $parent_section_id ) {
                $parent_section_id = FLAVOR_LIKE_SLUG;
            }
        ...
        public function get_values( $request = null ) {
            ...
            return apply_filters( 'flavor_like_optiwich_customizer_values', $values );
        }
        ...
        public function save_values( $request_or_values = null ) {
            ...
            if ( is_object( $request_or_values ) && method_exists( $request_or_values, 'get_json_params' ) ) {
                ...
                wp_send_json_error( array(
                    'message' => esc_html__( 'Invalid request data. Expected an object with customizer values.', 'flavor-like' ),
                ), 400 );
                return;
            }
            ...
            $values = apply_filters( 'flavor_like_optiwich_save_customizer_values', $values );
            ...
            do_action( 'flavor_like_customizer_saved', $values );

            wp_send_json_success( array(
                'message' => esc_html__( 'Customizer settings saved successfully.', 'flavor-like' ),
            ) );
        }
         ...
         * Returns rendered Flavor Like templates with current customizer styles
         */
        public function get_preview( $request = null ) {
            ...
            // Get Flavor Like front-end assets URLs using the same method as schema
            ...
            // Build full HTML with styles and Flavor Like assets
...
<head>
    ...
    <title>' . esc_html__( 'Flavor Like Customizer Preview', 'flavor-like' ) . '</title>
...
(function () {
  document.addEventListener("click", function (event) {
    var btn = event.target && event.target.closest
      ? event.target.closest(".ulp-customizer-button-preview .flavor_like_btn")
      : null;
    if (!btn) {
      return;
    }
    ...
    event.stopPropagation();
        ...
        * {
        ...
        protected function render_template_preview( $template_type ) {
            ...
            $preview_html = apply_filters( 'flavor_like_customizer_template_preview', '', $template_type );
            ...
            switch ( $template_type ) {
                ...
                default:
                    // Template not found
                    echo '<p class="flavor-like-preview-not-found">' . esc_html__( 'Template preview not found.', 'flavor-like' ) . '</p>';
            }
        ...
        protected function render_button_preview() {
            $templates = function_exists( 'flavor_like_generate_templates_list' )
                ? flavor_like_generate_templates_list()
                : array();
            ...
            add_filter( 'flavor_like_add_templates_args', $hide_likers, 999 );
            ...
            remove_filter( 'flavor_like_add_templates_args', $hide_likers, 999 );
        }
        ...
        protected function render_toast_preview() {
            $notification_classes = array(
                'flavorlike-notification',
            );
            ?>
            <div class="ulp-customizer-preview-root ulp-customizer-toast-preview">
                <p class="ulp-customizer-preview-label"><?php esc_html_e( 'Toast Messages', 'flavor-like' ); ?></p>
                <div class="<?php echo esc_attr( implode( ' ', $notification_classes ) ); ?>">
                    <div class="flavorlike-message">
                        <strong><?php esc_html_e( 'Info', 'flavor-like' ); ?></strong> <?php esc_html_e( 'Please wait...', 'flavor-like' ); ?>
                    </div>
                    <div class="flavorlike-message flavorlike-success">
                        <strong><?php esc_html_e( 'Success', 'flavor-like' ); ?></strong> <?php esc_html_e( 'You liked this post.', 'flavor-like' ); ?>
                    </div>
                    <div class="flavorlike-message flavorlike-error">
                        <strong><?php esc_html_e( 'Error', 'flavor-like' ); ?></strong> <?php esc_html_e( 'Something went wrong', 'flavor-like' ); ?>
                    </div>
                    <div class="flavorlike-message flavorlike-warning">
                        <strong><?php esc_html_e( 'Warning', 'flavor-like' ); ?></strong> <?php esc_html_e( 'Please check your settings.', 'flavor-like' ); ?>
                    </div>
                </div>
            </div>
            <?php
