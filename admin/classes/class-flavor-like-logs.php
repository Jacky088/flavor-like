...
if ( ! class_exists( 'flavor_like_logs' ) ) {

	class flavor_like_logs{
		// Private variables
		...
		function __construct( $identifier, $page = 1, $per_page = 15, $sort = array(
			...
			global $wpdb;
			$this->wpdb       = $wpdb;
			$this->identifier = Flavor_Like_Pulse_Registry::resolve_log_identifier( $identifier ) ?: $identifier;
			$this->page       = $page;
		...
		private function legacy_search_where() {
			...
			$source  = Flavor_Like_Pulse_Registry::legacy_source_for_type( $this->identifier );
		...
		public function get_results(){
			if ( flavor_like_use_pulse_queries() ) {
				return Flavor_Like_Pulse_Log_Bridge::get_log_rows(
					$this->identifier,
					$this->page,
					$this->per_page,
					$this->sort,
		...
		public function get_row( $item_ID ){
			if ( flavor_like_use_pulse_queries() ) {
				return Flavor_Like_Pulse_Log_Bridge::get_log_row( $this->identifier, $item_ID );
			}
		...
		public function get_all_rows(){
			if ( flavor_like_use_pulse_queries() ) {
				return Flavor_Like_Pulse_Log_Bridge::get_all_log_rows( $this->identifier, $this->sort, $this->search );
			}
		...
		public function delete_rows( $items ){
			if ( flavor_like_use_pulse_queries() ) {
				...
				if ( ! empty( $selected_ids ) ) {
					Flavor_Like_Pulse_Log_Bridge::delete_log_rows( $this->identifier, $selected_ids );
				}
				return;
			}
		...
		public function delete_row( $item_id ){
			if ( flavor_like_use_pulse_queries() ) {
				return Flavor_Like_Pulse_Log_Bridge::delete_log_row( $this->identifier, $item_id );
			}
		...
		private function get_total_records(){
			if ( flavor_like_use_pulse_queries() ) {
				return Flavor_Like_Pulse_Log_Bridge::count_log_rows( $this->identifier, $this->search );
			}
			...
			foreach ($dataset as $key => $row) {
				...
				if( isset( $row->user_id ) ){
					...
					if( $user_info ){
						...
						$output[$key]->user_id = '#'. esc_html__( 'Guest User', 'flavor-like' );
					}
				}
				...
				if( isset( $row->activity_id ) ){
					...
					// Activity title
					$activity_title = esc_html__('Activity Permalink','flavor-like');
				...
				if( isset( $row->comment_id ) ){
					...
					if( NULL != $comment ){
						...
						$output[$key]->comment_author  = $output[$key]->comment_content = esc_html__( 'Not Found!', 'flavor-like' );
					}
				}
			}

			return apply_filters( 'flavor_like_get_trnasformed_rows', $output );
		}
