...
if ( ! class_exists( 'flavor_like_settings_api' ) ) {
    class flavor_like_settings_api {
        ...
        public function get_schema( $request = null ) {
            ...
            return apply_filters( 'flavor_like_optiwich_schema', $schema );
        }
        ...
        protected function build_schema_from_panel() {
            ...
            if ( ! did_action( 'flavor_like_settings_loaded' ) ) {
                do_action( 'flavor_like_settings_loaded' );
            }
        ...
        public function get_optiwich_schema() {
            ...
            global $flavor_like_admin_panel;

            // If admin panel doesn't exist, try to create it
            if ( ! isset( $flavor_like_admin_panel ) && class_exists( 'flavor_like_admin_panel' ) ) {
                $flavor_like_admin_panel = new flavor_like_admin_panel();
            }

            // Get sections from admin panel
            if ( isset( $flavor_like_admin_panel ) && method_exists( $flavor_like_admin_panel, 'register_sections' ) ) {
                $sections = $flavor_like_admin_panel->register_sections();
            } else {
                $sections = array();
            }
            ...
            return array( 'pages' => apply_filters( 'flavor_like_optiwich_pages', $pages ) );
        }
                    ...
                    if ( isset( $field['options'] ) && is_string( $field['options'] ) ) {
                        ...
                        if ( ! empty( $resolved ) ) {
                            ...
                            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                                error_log( sprintf(
                                    '[Flavor Like] Failed to resolve select options for field "%s" with type "%s". Check if the function exists and is callable.',
                                    $field['id'] ?? 'unknown',
                                    $field['options']
                                ) );
                            }
        ...
        protected function get_values() {
            ...
            return apply_filters( 'flavor_like_optiwich_values', $values );
        }
        ...
        public static function get_select_options( $type, $term = '', $query_args = array() ) {
            ...
            switch( $type ) {
                ...
                default:
                    // Handle custom callable functions (like flavor_like_pro_search_attachments)
                    if ( is_callable( $type ) ) {
        ...
        public function save_settings( $request_or_values = null ) {
            ...
            if ( is_object( $request_or_values ) && method_exists( $request_or_values, 'get_json_params' ) ) {
                ...
                return new WP_Error(
                    'invalid_data',
                    esc_html__( 'Invalid request data. Expected an object with setting values.', 'flavor-like' ),
                    array( 'status' => 400 )
                );
            }
            ...
            $values = apply_filters( 'flavor_like_optiwich_save_values', $values );
            ...
            do_action( 'flavor_like_settings_saved', $values );

            return array(
                'success' => true,
                'message' => esc_html__( 'Settings saved successfully.', 'flavor-like' ),
            );
        }
        ...
        protected function sanitize_settings_values( $values ) {
            if ( is_array( $values ) ) {
                ...
                if ( $this->contains_html( $values ) ) {
                    // String contains HTML - use flavor_like_kses for safe HTML sanitization
                    return flavor_like_kses( $values );
                } else {
        ...
        protected function build_child_page( $section ) {
            ...
            if ( ! isset( $section['id'] ) || empty( $section['id'] ) ) {
                ...
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( sprintf(
                        '[Flavor Like] Section "%s" is missing explicit "id" field. Generated ID: "%s". Please add explicit ASCII ID in admin panel.',
        ...
        public function get_translations() {
            $translations = array(
                ...
                'errors.generic' => esc_html__( 'Something went wrong', 'flavor-like' ),
                /* translators: Error message with suggestion to refresh the page */
                'errors.generic_refresh' => esc_html__( 'Something went wrong. Please try refreshing the page.', 'flavor-like' ),
                /* translators: Error message when a request fails */
                'errors.failed' => esc_html__( 'Unable to complete request', 'flavor-like' ),
                /* translators: Error message when server connection is forbidden */
                'errors.admin_ajax_forbidden' => esc_html__( 'Unable to connect to server. Please contact your administrator.', 'flavor-like' ),
                /* translators: Error message when server endpoint is not found */
                'errors.admin_ajax_not_found' => esc_html__( 'Server endpoint not found. Please check your configuration.', 'flavor-like' ),
                /* translators: Generic server error message */
                'errors.server_error' => esc_html__( 'Server error. Please try again later.', 'flavor-like' ),
                /* translators: Network connection error message */
                'errors.network_error' => esc_html__( 'Unable to connect to server. Please check your internet connection and try again.', 'flavor-like' ),
                /* translators: CORS (Cross-Origin Resource Sharing) error message */
                'errors.cors_error' => esc_html__( 'Connection blocked. Please contact your administrator.', 'flavor-like' ),
                /* translators: Request timeout error message */
                'errors.timeout' => esc_html__( 'Request timed out. Please try again.', 'flavor-like' ),
                /* translators: Error when settings configuration cannot be loaded */
                'errors.no_schema' => esc_html__( 'Unable to load settings. Configuration data is missing.', 'flavor-like' ),

                // Actions
                /* translators: Button text to save changes */
                'actions.save' => esc_html__( 'Save Changes', 'flavor-like' ),
                /* translators: Short button text to save changes (compact toolbar) */
                'actions.save_short' => esc_html__( 'Save', 'flavor-like' ),
                /* translators: Loading state text while saving */
                'actions.saving' => esc_html__( 'Saving...', 'flavor-like' ),
                /* translators: Button text to reset settings to default values */
                'actions.reset' => esc_html__( 'Reset to Defaults', 'flavor-like' ),
                /* translators: Short button text to reset settings (compact toolbar) */
                'actions.reset_short' => esc_html__( 'Reset', 'flavor-like' ),
                /* translators: Loading state text while resetting */
                'actions.resetting' => esc_html__( 'Resetting...', 'flavor-like' ),
                /* translators: Button text to remove an item */
                'actions.remove' => esc_html__( 'Remove', 'flavor-like' ),
                /* translators: Button text to upload a file */
                'actions.upload' => esc_html__( 'Upload', 'flavor-like' ),
                /* translators: Button text to import settings */
                'actions.import' => esc_html__( 'Import', 'flavor-like' ),
                /* translators: Loading state text while importing */
                'actions.importing' => esc_html__( 'Importing...', 'flavor-like' ),
                /* translators: Button text to export and download settings */
                'actions.export' => esc_html__( 'Export and Download', 'flavor-like' ),
                /* translators: Button text to add an item. %s will be replaced with the item type */
                'actions.add' => esc_html__( 'Add %s', 'flavor-like' ),

                // Media Library
                /* translators: Button text to select media. %s will be replaced with media type (e.g., "image", "video") */
                'media.select' => esc_html__( 'Select %s', 'flavor-like' ),
                /* translators: Button text to use selected media. %s will be replaced with media type */
                'media.use' => esc_html__( 'Use this %s', 'flavor-like' ),
                /* translators: Error when selected media has no URL. %s will be replaced with media type */
                'media.no_url' => esc_html__( 'Selected %s has no URL', 'flavor-like' ),
                /* translators: Message when no file is selected */
                'media.no_selection' => esc_html__( 'No file selected', 'flavor-like' ),
                /* translators: Error message for invalid URL format from media library */
                'media.url_format_error' => esc_html__( 'Invalid URL format from media library', 'flavor-like' ),

                // Settings
                /* translators: Success message after settings action. %s will be replaced with the action (e.g., "saved", "reset") */
                'settings.success' => '设置已保存成功。',
                /* translators: Success message after resetting settings to defaults */
                'settings.reset_success' => '设置已恢复为默认值。',
                /* translators: Confirmation dialog text before resetting all settings */
                'settings.reset_confirm' => '将所有设置恢复为默认值？此操作无法撤消。',
                /* translators: Warning message when user tries to leave with unsaved changes */
                'settings.unsaved_warning' => '您有未保存的更改。确定要离开吗？',
                ...
                'settings.save_hint' => esc_html__( 'Save when you are done editing these settings.', 'flavor-like' ),

                // Settings — sidebar search (user-visible copy)
                /* translators: Placeholder text in the settings sidebar search field */
                'settings.search_placeholder' => esc_html__( 'Search settings...', 'flavor-like' ),
                /* translators: Message shown when settings search returns no matches */
                'settings.search_no_results' => esc_html__( 'No settings found.', 'flavor-like' ),
                ...
                'settings.import_save_failed' => esc_html__( 'Settings imported locally but failed to save to server. Please try saving again.', 'flavor-like' ),

                // Validation
                /* translators: 1: field type, 2: optional example suffix (e.g. ". (e.g., https://example.com)") */
                'validation.invalid' => esc_html__( 'Invalid %1$s format%2$s', 'flavor-like' ),
                /* translators: Error message for invalid URL protocol */
                'validation.url_protocol' => esc_html__( 'Invalid URL protocol. Only http, https, and data URLs are allowed.', 'flavor-like' ),
                /* translators: Validation error for text exceeding maximum length. %d will be replaced with maximum length */
                'validation.text_maxlength' => esc_html__( 'Text must be no more than %d characters', 'flavor-like' ),
                /* translators: Validation error for number below minimum. %d will be replaced with minimum value */
                'validation.number_min' => esc_html__( 'Value must be at least %d', 'flavor-like' ),
                /* translators: Validation error for number above maximum. %d will be replaced with maximum value */
                'validation.number_max' => esc_html__( 'Value must be at most %d', 'flavor-like' ),
                /* translators: Validation error when upload field requires a URL string */
                'validation.upload_url_required' => esc_html__( 'Upload value must be a URL string', 'flavor-like' ),
                /* translators: Validation error for media field format */
                'validation.media_format' => esc_html__( 'Media value must be an object with a url property or a valid URL string', 'flavor-like' ),

                // Backup/Import
                /* translators: Title for import settings section */
                'backup.import_title' => esc_html__( 'Import Settings', 'flavor-like' ),
                /* translators: Description text for import settings feature */
                'backup.import_desc' => esc_html__( 'Paste your exported settings JSON below and click Import to restore your configuration. The import should contain only setting values (not schema structure).', 'flavor-like' ),
                /* translators: Placeholder text for import JSON textarea */
                'backup.import_placeholder' => esc_html__( 'Paste your JSON settings here...', 'flavor-like' ),
                /* translators: Title for export settings section */
                'backup.export_title' => esc_html__( 'Export Settings', 'flavor-like' ),
                /* translators: Description text for export settings feature */
                'backup.export_desc' => esc_html__( 'Copy the JSON below or download it as a file to backup your current settings. The export contains only your setting values (not the schema structure).', 'flavor-like' ),
                /* translators: Error message for invalid JSON format */
                'backup.json_invalid' => esc_html__( 'Invalid JSON format. Please check your JSON syntax.', 'flavor-like' ),
                /* translators: Error when no values found in imported data */
                'backup.no_values' => esc_html__( 'No values found in the import data.', 'flavor-like' ),
                /* translators: Security warning when dangerous content detected. %s will be replaced with threat list */
                'backup.security_threat' => esc_html__( "Security threat detected. The import contains potentially dangerous content:\n%s\n\nImport blocked for security reasons.", 'flavor-like' ),
                /* translators: Validation error when imported data has invalid values. %s will be replaced with error list */
                'backup.validation_failed' => esc_html__( "Validation failed. The imported data contains invalid values:\n%s\n\nPlease fix these errors and try again.", 'flavor-like' ),

                // Code Editor
                /* translators: Tip text for code editor usage */
                'code_editor.tip' => esc_html__( 'Tip: Select text to wrap it, or click to insert at cursor position', 'flavor-like' ),
                /* translators: Tab label for visual editor mode */
                'code_editor.visual' => esc_html__( 'Visual', 'flavor-like' ),
                /* translators: Tab label for text editor mode */
                'code_editor.text' => esc_html__( 'Text', 'flavor-like' ),
                /* translators: Tab label for preview mode */
                'code_editor.preview' => esc_html__( 'Preview', 'flavor-like' ),
                /* translators: Message when there is no content to preview */
                'code_editor.no_content' => esc_html__( 'No content to preview', 'flavor-like' ),

                // Select Field
                /* translators: Placeholder text for multi-select dropdown */
                'select.placeholder_multiple' => esc_html__( 'Select options...', 'flavor-like' ),
                /* translators: Placeholder text for single-select dropdown */
                'select.placeholder_single' => esc_html__( 'Select an option', 'flavor-like' ),

                // Field Errors
                /* translators: Error when no options are available for a field */
                'field.no_options' => esc_html__( 'No options available for this field. Please check the schema definition.', 'flavor-like' ),
                /* translators: Error when dynamic options cannot be resolved. Simplified message for users */
                'field.options_unresolved' => esc_html__( 'Options not available. Please check your configuration.', 'flavor-like' ),

                // Security
                /* translators: Security threat type name - SQL Injection */
                'security.sql_injection' => esc_html__( 'SQL Injection', 'flavor-like' ),
                /* translators: Security threat type name - XSS (Cross-Site Scripting) */
                'security.xss' => esc_html__( 'XSS', 'flavor-like' ),
                /* translators: Security threat type name - Command Injection */
                'security.command_injection' => esc_html__( 'Command Injection', 'flavor-like' ),

                // Common UI
                /* translators: Button text to retry a failed action */
                'ui.retry' => esc_html__( 'Retry', 'flavor-like' ),
                /* translators: Label for live preview feature */
                'ui.live_preview' => esc_html__( 'Live Preview', 'flavor-like' ),
                /* translators: Error message when preview fails to load */
                'ui.failed_to_load_preview' => esc_html__( 'Failed to load preview', 'flavor-like' ),

                // Customizer (user-visible copy only; aria/sr-only labels use React fallbacks)
                /* translators: Label for templates section */
                'customizer.templates' => esc_html__( 'Templates', 'flavor-like' ),
                /* translators: Heading for template list sidebar */
                'customizer.list_view' => esc_html__( 'List View', 'flavor-like' ),
                /* translators: Tab label for section settings panel */
                'customizer.section_settings' => esc_html__( 'Section', 'flavor-like' ),
                /* translators: Mobile back button label */
                'customizer.back_to_preview' => esc_html__( 'Back to preview', 'flavor-like' ),
                /* translators: Message when no preview templates are available */
                'customizer.no_preview_templates' => esc_html__( 'No preview templates available', 'flavor-like' ),
                /* translators: Instruction text for selecting a template section */
                'customizer.select_section' => esc_html__( 'Select a template section to customize', 'flavor-like' ),

                // Common Field Labels (used across multiple field types)
                /* translators: Label for width field */
                'field.width' => esc_html__( 'Width', 'flavor-like' ),
                /* translators: Label for height field */
                'field.height' => esc_html__( 'Height', 'flavor-like' ),
                /* translators: Label for color field */
                'field.color' => esc_html__( 'Color', 'flavor-like' ),
                /* translators: Label for style field */
                'field.style' => esc_html__( 'Style', 'flavor-like' ),
                ...
                'general.options' => esc_html__( 'Options', 'flavor-like' ),
                /* translators: Generic label for general section */
                'general.general' => esc_html__( 'General', 'flavor-like' ),
                /* translators: Generic label for settings */
                'general.settings' => esc_html__( 'Settings', 'flavor-like' ),
                /* translators: Generic label for new item */
                'general.new' => esc_html__( 'New', 'flavor-like' ),
                /* translators: Generic label for item */
                'general.item' => esc_html__( 'Item', 'flavor-like' ),

                // Emoji reactions field
                'fields.quick_add' => esc_html__( 'Quick add', 'flavor-like' ),
                'fields.emoji' => esc_html__( 'Emoji', 'flavor-like' ),
                'fields.label' => esc_html__( 'Label', 'flavor-like' ),
                'fields.reaction_label_placeholder' => esc_html__( 'Like', 'flavor-like' ),
                'fields.add_reaction' => esc_html__( 'Add reaction', 'flavor-like' ),
                'fields.choose_emoji' => esc_html__( 'Choose emoji', 'flavor-like' ),
