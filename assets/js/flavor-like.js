...
/* ================== assets/js/src/flavor_like.js =================== */


/**
 * Flavor Like - Main Plugin
 * 
...
(function (window, document, undefined) {
  ...
  const defaults = {
    ...
    generalSelector: ".flavor_like_general_class",
    buttonSelector: ".flavor_like_btn",
    likersSelector: ".flavor_like_likers_wrapper",
  };
  const attributesMap = {
    "flavor-like-id": "ID",
    "flavor-like-nonce": "nonce",
    "flavor-like-type": "type",
    "flavor-like-append": "append",
    "flavor-like-is-total": "isTotal",
    "flavor-like-display-likers": "displayLikers",
    "flavor-like-likers-style": "likersTemplate",
    "flavor-like-disable-pophover": "disablePophover",
    "flavor-like-append-timeout": "appendTimeout",
    "flavor-like-factor": "factor",
    "flavor-like-template": "template",
  };
  ...
  function Plugin(element, options) {
    ...
    if (this.counterElement.length > 0) {
      forEachElement(this.counterElement, (element) => {
        const counterValue = getDataAttribute(element, "flavor-like-counter-value");
  ...
  Plugin.prototype = {
    ...
    _ajax(args, callback) {
      ...
      fetch(flavor_like_params.ajax_url, {
        method: "POST",
        body: formData,
      })
        .then((response) => {
        ...
        .then(callback)
        .catch((error) => {
          console.error("Flavor Like AJAX error:", error);
          if (self.generalElement) {
            forEachElement(self.generalElement, (el) => {
              el.classList.remove("flavor_like_is_loading");
              el.classList.remove("flavor_like_is_getting_likers_list");
            });
          }
          ...
          if (
            flavor_like_params &&
            flavor_like_params.notifications &&
            flavor_like_params.ajax_error
          ) {
            self._sendNotification("error", flavor_like_params.ajax_error);
          }
          triggerEvent(document, "FlavorLikeUpdated", self.element);
        });
    },

    /**
     * init flavor_like core process
     */
    _initLike(event) {
      ...
      // Manipulations
      triggerEvent(document, "FlavorLikeLoading", this.element);
      // Add progress class
      if (this.generalElement) {
        forEachElement(this.generalElement, (el) => {
          el.classList.add("flavor_like_is_loading");
        });
      }
      // Start AJAX process
      this._ajax(
        {
          action: "flavor_like_process",
          id: this.settings.ID,
        ...
        (response) => {
          ...
          triggerEvent(document, "FlavorLikeUpdated", this.element);
        }
      );
    },
    ...
    _maybeUpdateElements(event) {
      ...
      this.settings.factor = getDataAttribute(this.buttonElement, "flavor-like-factor");
    },
    ...
    _updateGeneralClassNames(status) {
      const classNameObj = {
        start: "flavor_like_is_not_liked",
        active: "flavor_like_is_liked",
        deactive: "flavor_like_is_unliked",
        disable: "flavor_like_click_is_disabled",
      };
    ...
    __updateCounter(counterValue) {
      ...
      forEachElement(this.counterElement, (element) => {
        element.setAttribute("data-flavor-like-counter-value", counterValue);
        element.innerHTML = counterValue;
      });

      const buttonEl = getSingleElement(this.buttonElement);
      triggerEvent(document, "FlavorLikeCounterUpdated", [buttonEl]);
    },
    ...
    _fetchLikersData() {
      ...
      if (generalEl) {
        generalEl.classList.add("flavor_like_is_getting_likers_list");
      }

      this._ajax(
        {
          action: "flavor_like_get_likers",
          id: this.settings.ID,
          nonce: this.settings.nonce,
          type: this.settings.type,
          displayLikers: this.settings.displayLikers,
        ...
        (response) => {
          if (generalEl) {
            generalEl.classList.remove("flavor_like_is_getting_likers_list");
          }
          this._isFetchingLikers = false;
          if (response.success) {
    ...
    _getAllTooltipElements() {
      ...
      forEachElement(allSameButtons, (btn) => {
        const wrapper = btn.closest('.flavorlike');
    ...
    _updateLikers(event) {
      if (this.settings.displayLikers) {
        ...
        if (
          ...
          getDataAttribute(this.element, "flavor-like-tooltip")
        ) {
          return;
        } else if (
        ...
        if (this.settings.likersTemplate === "popover") {
          if (typeof FlavorLikeTooltipPlugin !== "undefined") {
            ...
            const currentInstance = window.FlavorLikeTooltip && window.FlavorLikeTooltip.getInstanceByElement
              ? window.FlavorLikeTooltip.getInstanceByElement(this.element)
              : null;

            if (!currentInstance) {
              new FlavorLikeTooltipPlugin(this.element, {
                id: tooltipId,
                position: "top",
    ...
    _updateLikersMarkup(data) {
      ...
      const template = data && typeof data === 'object' ? data.template : data;
      triggerEvent(document, "FlavorLikeLikersMarkupUpdated", [
        this.likersElement,
    ...
    _updateButton(btnText, status) {
      forEachElement(this.buttonElement, (buttonEl) => {
        ...
        if (buttonEl.classList.contains("flavor_like_put_image")) {
          if (status === 4) {
            buttonEl.classList.add("image-unlike", "flavor_like_btn_is_active");
          } else {
            buttonEl.classList.toggle("image-unlike");
            buttonEl.classList.toggle("flavor_like_btn_is_active");
          }
        } else if (
          buttonEl.classList.contains("flavor_like_put_text") &&
          btnText !== null
        ) {
      ...
      if (this.siblingElement && this.siblingElement.length) {
        forEachElement(this.siblingElement, (sibling) => {
          ...
          if (siblingBtn) {
            siblingBtn.classList.remove("image-unlike", "flavor_like_btn_is_active");
          }
        });
      }
    ...
    /**
     * Send notification by 'FlavorLikeNotifications' plugin
     */
    _sendNotification(messageType, messageText) {
      if (typeof FlavorLikeNotifications !== "undefined") {
        new FlavorLikeNotifications(document.body, {
          messageType,
          messageText,
        });
      }
  ...
  if (typeof jQuery !== 'undefined' && jQuery && jQuery.fn) {
    jQuery.fn[pluginName] = function (options) {
      return this.each(function () {
        if (!this.hasAttribute || !this.hasAttribute("data-flavor-like-initialized")) {
          try {
            new Plugin(this, options);
            if (this.setAttribute) {
              this.setAttribute("data-flavor-like-initialized", "true");
            }
          } catch (err) {
            if (window.console && typeof window.console.error === "function") {
              window.console.error("Flavor Like: failed to initialize button", this, err);
            }
            if (this && this.setAttribute) {
              this.setAttribute("data-flavor-like-initialized", "error");
            }
          }
        }
      });
